...
fn test_jump() {
    ...
    let mut s = Simulator::new(&config, None).unwrap();
...
fn test_random_provided() {
    ...
    let mut s = Simulator::new(&COSMAC_VIP, Some(random_values)).unwrap();
    s.load_program(&prog).unwrap();
...
fn test_bcd_font() {
    ...
    // : the_end
    // jump the_end
    let prog = [0x12, 0x05, 0x00, 0x00, 0x00, 0xA2, 0x02, 0x64, 0x7B, 0xF4, 0x33, 0xF2, 0x65,
                0x65, 0x00, 0x66, 0x00, 0xF0, 0x29, 0xD5, 0x65, 0x65, 0x05, 0xF1, 0x29, 0xD5,
...
fn test_equality_literal() {
    ...
    // : the_end
    // jump the_end
    let prog = [0x60, 0x00, 0x70, 0x01, 0x30, 0x0A, 0x12, 0x02, 0x40, 0x0A, 0x61, 0x01, 0x40,
                0x0B, 0x62, 0x01, 0x12, 0x10];
    let mut s = Simulator::new(&COSMAC_VIP, None).unwrap();
    s.load_program(&prog).unwrap();
